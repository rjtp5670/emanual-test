...
1. An Instruction from the user is transmitted via DYNAMIXEL bus, then registered to [Goal Velocity(104)](#goal-velocity-{{ passed_model }}){: .popup2}.
2. Goal Velocity(104) is converted to desired velocity trajectory by [Profile Acceleration(108)](#profile-acceleration-{{ passed_model }}){: .popup2}.
3. The desired velocity trajectory is stored at [Velocity Trajectory(136)](#velocity-trajectory-{{ passed_model }}){: .popup2}.
